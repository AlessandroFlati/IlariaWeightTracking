...
2. **`src/models.py::fit_all_models`** — fits three models against `(days, weights)`: linear (via `np.polyfit`), Hall linearized 2011 `w_final + (w0-w_final)·exp(-t/τ)`, and Thomas 2013 `w0 - Δw·(1-exp(-k·t))` (via `scipy.optimize.curve_fit`, `maxfev=10000`). Each model returns `{"predict": callable, "r_squared": float, "residual_std": float}`. Non-linear fits that raise `RuntimeError` are silently dropped from the result dict — callers must not assume Hall/Thomas keys exist.
