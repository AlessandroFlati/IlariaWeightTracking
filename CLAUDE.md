...
2. **`src/models.py::fit_all_models`** — fits two models against `(days, weights)`: linear (via `np.polyfit`) and Thomas 2013 `w0 - Δw·(1-exp(-k·t))` (via `scipy.optimize.curve_fit`, `maxfev=10000`). Each model returns `{"predict": callable, "r_squared": float, "residual_std": float}`. Thomas fit can raise `RuntimeError` and is silently dropped from the result dict — callers must not assume the Thomas key exists.
